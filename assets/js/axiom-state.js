/** GNOSTIRI — canonical product state and compound isolation. */
...
export function assertDomain(domain){if(!isValidDomain(domain))throw new Error(`Invalid GNOSTIRI domain: ${domain}`);return state.domain===domain;}
export function scopeKey(domain,id){if(!isValidDomain(domain))throw new Error(`Invalid GNOSTIRI domain: ${domain}`);return `${domain}::${String(id)}`;}
export function canAccessDomain(domain){return isValidDomain(domain)&&state.domain===domain;}
export function subscribe(listener){listeners.add(listener);listener(getState());return()=>listeners.delete(listener);}
window.GnostiriState=Object.freeze({DOMAINS,ACTIVITIES,getState,setDomain,setActivity,setState,assertDomain,scopeKey,canAccessDomain,subscribe});
persist();
