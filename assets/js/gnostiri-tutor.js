/**
 * GNOSTIRI Tutor — personal adaptive study planning layer.
 * Local-first: no API key, no external AI call.
 * Plans are domain-scoped and automatically refreshed from progress events.
 */
...
export class GnostiriTutor {
  ...
  getInsights(domain=activeDomain()) {
    ...
    const strong = breakdown.filter(x=>x.accuracy>=75).slice(0,5);
    return { summary, breakdown, weak, practice, strong };
  }
  ...
  generatePlan(domain=activeDomain()) {
    ...
    const minutes=Math.max(20, Math.min(720, Number(profile.dailyMinutes)||60));
    ...
    for(let i=0;i<days;i++){
      ...
      const review=Math.max(5,Math.round(minutes*.2));
      const quiz=Math.max(5,Math.round(minutes*.3));
      const study=Math.max(10,minutes-review-quiz);
      plan.push({day:i+1,date:new Date(Date.now()+i*86400000).toISOString().slice(0,10),focus,accuracy,tasks:[
        {type:"study",label:`Study: ${label(focus)}`,minutes:study},
        {type:"quiz",label:`Practice: ${label(focus)}`,minutes:quiz},
        {type:"review",label:"Review mistakes & recall",minutes:review}
      ],completed:false});
    }
    return {domain,generatedAt:Date.now(),examName:profile.examName||"Study goal",examDate:profile.examDate||null,dailyMinutes:minutes,plan,source:{weak:insights.weak.length,practice:insights.practice.length,strong:insights.strong.length}};
  }
...
export function renderTutorDashboard(root){
  ...
  const weak=insights.weak.length ? insights.weak.map(x=>`<span>${label(x.topic)} · ${x.accuracy}%</span>`).join('') : '<span>No weak topics detected yet</span>';
  const completion=plan.plan.length ? Math.round(plan.plan.filter(d=>d.completed).length/plan.plan.length*100) : 0;
  const daysLeft=p.examDate ? daysBetween(todayKey(),p.examDate) : null;
  ...
  <section class="tutor-grid">
    <article class="tutor-card tutor-today"><div class="tutor-card-head"><div><small>TODAY'S PRIORITY</small><h2>${today?.focus?label(today.focus):'Set a study goal'}</h2></div><strong>${today?.completed?'Done':'Next'}</strong></div>${today?`<ul>${today.tasks.map(t=>`<li><span>${t.label}</span><b>${t.minutes} min</b></li>`).join('')}</ul><button id="tutor-done" class="tutor-primary" ${today.completed?'disabled':''}>${today.completed?'Today's plan completed':'Complete today\'s plan'}</button>`:'<p>Set your examination date and available study time to create a plan.</p>'}</article>
    <article class="tutor-card"><small>LEARNING SIGNALS</small><div class="tutor-stats"><div><b>${insights.summary.avgAccuracy}%</b><span>Accuracy</span></div><div><b>${insights.summary.totalQuizzesTaken}</b><span>Quizzes</span></div><div><b>${insights.summary.streakDays}</b><span>Day streak</span></div></div><h3>Priority topics</h3><div class="tutor-chips">${weak}</div></article>
  </section>
  <section class="tutor-card tutor-plan-card"><div class="tutor-card-head"><div><small>ADAPTIVE PLAN</small><h2>${label(p.examName||'Your study plan')}</h2></div><span class="tutor-countdown">${daysLeft?`${daysLeft} days to target`:'Self-paced'}</span></div><div class="tutor-progress"><div><span>Plan completion</span><b>${completion}%</b></div><div class="tutor-progress-track"><i style="width:${completion}%"></i></div></div><div class="tutor-plan">${plan.plan.slice(0,7).map(d=>`<div class="tutor-day ${d.completed?'is-done':''}"><div><b>Day ${d.day}</b><small>${d.date}</small></div><span>${label(d.focus)}</span><strong>${d.completed?'✓':'→'}</strong></div>`).join('')}</div></section>
  <section class="tutor-card"><small>PLAN SETTINGS</small><form id="tutor-form" class="tutor-form"><label>Exam / goal<input id="tutor-exam" value="${p.examName||''}" placeholder="e.g. JAMB 2027"></label><label>Target date<input id="tutor-date" type="date" value="${p.examDate||''}"></label><label>Minutes per day<input id="tutor-minutes" type="number" min="20" max="720" value="${p.dailyMinutes||60}"></label><button class="tutor-primary" type="submit">Save & update plan</button></form><p class="tutor-note">Your plan is regenerated when your quiz performance changes.</p></section>`;
  root.querySelector('#tutor-form')?.addEventListener('submit',e=>{e.preventDefault();gnostiriTutor.saveProfile({domain,examName:root.querySelector('#tutor-exam').value.trim(),examDate:root.querySelector('#tutor-date').value,dailyMinutes:root.querySelector('#tutor-minutes').value});gnostiriTutor.updatePlanFromProgress(domain);renderTutorDashboard(root);});
  root.querySelector('#tutor-done')?.addEventListener('click',()=>{gnostiriTutor.markTodayComplete(domain);renderTutorDashboard(root);});
}

document.addEventListener('gnostiri:progress-updated',event=>{
  const domain=event.detail?.domain;
  if(!domain || domain===activeDomain()){
    gnostiriTutor.updatePlanFromProgress(activeDomain());
    renderTutorDashboard(document.getElementById('gnostiri-tutor'));
  }
});
